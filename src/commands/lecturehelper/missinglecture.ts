...
export async function handleMissingLectureManually(courseId: string, dateStr: string): Promise<EmbedBuilder | string> {
	const canvasToken = CANVAS.TOKEN;
	const lectureDate = new Date(dateStr);
	const weekStart = new Date(lectureDate);
	weekStart.setDate(weekStart.getDate() - weekStart.getDay());
	...
	try {
		...
		if (weeklyAssignments.length > 0) {
			assignmentList = weeklyAssignments
				.map((a, i) => `(${i + 1} of ${weeklyAssignments.length}) 🔗 [${a.name}](${a.html_url}) (Due: <t:${Math.floor(new Date(a.due_at).getTime() / 1000)}:F>)`
				)
				.join('\n');
		}
		// Final embed
